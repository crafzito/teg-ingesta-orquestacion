...
function centroFilter(soc: Sociedad): string {
  if (!soc) return ''
  return ` AND centro::text LIKE '${soc}%'`
}

export function kpiVentasMes(soc: Sociedad): string {
  return `SELECT COALESCE(SUM(monto_usd), 0) as total FROM public.v_ventas WHERE date_trunc('month', fecha_doc) = date_trunc('month', CURRENT_DATE)${socFilter(soc)}`
}

export function kpiCxcTotal(soc: Sociedad): string {
  return `SELECT COALESCE(SUM(valor_monetario), 0) as total FROM public.v_cxc WHERE 1=1${socFilter(soc)}`
}

export function kpiCxcVencida(soc: Sociedad): string {
  return `SELECT COALESCE(SUM(total_vencido), 0) as total FROM public.v_cxc WHERE 1=1${socFilter(soc)}`
}

export function kpiInventarioValor(soc: Sociedad): string {
  return `SELECT COALESCE(SUM(valor_libre), 0) as total FROM public.v_inventario WHERE 1=1${centroFilter(soc)}`
}

export function kpiOrdenesActivas(soc: Sociedad): string {
  return `SELECT COUNT(*) as total FROM public.v_ordenes WHERE estatus IN ('Abiertos', 'Liberados')${centroFilter(soc)}`
}
...
export function chartTopClientes(soc: Sociedad): string {
  return `
    ...
    FROM public.v_ventas
    WHERE date_trunc('month', fecha_doc) >= date_trunc('month', CURRENT_DATE) - interval '3 months'${socFilter(soc)}
    GROUP BY nombre
    ORDER BY total DESC
    LIMIT 10
  `
